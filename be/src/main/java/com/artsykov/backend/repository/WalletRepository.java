...
public interface WalletRepository extends CrudRepository<WalletEntity, Integer> {
    ...
    @Query(value = "SELECT wallet.id_wallet, balance, card_number, card_date, card_cvv_code, person_name, debt" +
            " FROM backend.wallet INNER JOIN customer on customer.id_wallet = wallet.id_wallet" +
            " WHERE id_customer = ?1", nativeQuery=true)
    WalletEntity findWalletEntityByIdCustomer(@Param("id_customer") int id);

    @Query(value = "SELECT wallet.id_wallet, balance, card_number, card_date, card_cvv_code, person_name, debt" +
            " FROM backend.wallet INNER JOIN company on company.id_wallet = wallet.id_wallet " +
